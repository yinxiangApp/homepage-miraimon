...
Tree.prototype.generateHtmlFromData = function (data) {
  ...
  for (var ii = 0; ii < data[0].length; ++ii) {
    ...
    if (!this.options.onlyBatchSelection) {
      ...
      optionCheckboxStr += "/>";
      optionLabelStr += '<label class=\'item-label\' for=\'' + optionLabelCheckboxId + '\'>' + (option.text || option.value) + '</label>';
    } else {
      optionLabelStr += '' + (option.text || option.value);
    }
    var descriptionPopupStr = option.description ? "<span class='description'>?</span>" : "";

    str += '<div class=\'item checkbox\' data-key=\'' + option.id + '\'data-value=\'' + option.value + '\'' + descriptionStr + indexStr + '>' + optionCheckboxStr + descriptionPopupStr + optionLabelStr + '</div>';
  }
  ...
  for (var jj = 0; jj < keys.length; ++jj) {
    ...
    str += '<div class=\'section\'><div class=\'title checkbox\'>' + sectionCheckboxStr + '<label class=\'title-left-padding\'>' + keys[jj] + '</label></div>' + generatedData + '</div>';
  }
  return str;
};
